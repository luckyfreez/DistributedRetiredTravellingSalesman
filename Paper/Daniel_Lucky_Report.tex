...
This section addresses a number of issues briefly touched upon in Section~\ref{sec:int_prog_form} and other areas of this paper.
...
hubs such as Chicago O'Hare or Hartsfield-Jackson Atlanta.
...
our code might output \texttt{[ALB->ORD, 07/07/2014, \$400]}, there will be another flight ``hidden'' in our route.
...
cities. For example, if we wanted to travel to Chicago, Atlanta, and Denver in some order, the cheapest route 
...
{\bf Details on crawler...}

In general, our crawler is safe enough that we are confident in running it and only periodically checking back on its results. There are still several
points of concern related to the crawler.

{\bf Talk about my concerns...}
...
\begin{itemize}[noitemsep]
    \item We could address the limitations we discussed in Section~\ref{sec:limitations}.
